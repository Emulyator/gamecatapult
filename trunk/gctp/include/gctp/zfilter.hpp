...
namespace gctp {
	...
	protected:
		...
		/*virtual pos_type seekoff(off_type off, std::ios_base::seekdir dir, std::ios_base::openmode mode = std::ios_base::in | std::ios_base::out)
		{
			...
			return streambuf_->pubseekoff(off, dir, mode);
		}
		...
		void decompress()
		{
			...
			for(;;){
				if(zs.inputAvail() == 0){
					// 入力バッファが尽きた
					zs.setInput((unsigned char*)&deflated_.front(), (unsigned int)(streambuf_->sgetn(&deflated_.front(), (std::streamsize)deflated_.size())*sizeof(Ch)));
					if(zs.inputAvail() == 0){
