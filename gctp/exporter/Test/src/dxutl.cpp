 ...
 *
 */
void DX_MakeNodeName( char* dst, const char* src )
{
	while (*src) {
		if (!(*src >= 'a' && *src <= 'z') &&
			!(*src >= 'A' && *src <= 'Z') &&
 ...
 *
 */
void DX_FileBodyName( char* dst, const char* src )
{
	PathConvParam   param;

	param.path_in = const_cast<char *>(src);
	param.name    = dst;
 ...
 *
 */
void DX_TextureName( char* dst, const char* src, const char* ext )
{
	PathConvParam   param;
	DX_FNAM         temp;
 ...
 *
 */
void DX_FileUniqName( char* dst, const char* src )
{
	PathConvParam   param;
	char*           ext;
