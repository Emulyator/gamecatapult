...
#ifndef GCTP_USE_XFILEAPI
static void  UI_TEMP_Event( LWControl* control, dxUI* ui );
#endif
...
int dxUI::Setup()
{
...
#ifndef GCTP_USE_XFILEAPI
	_control_main[UI_MAIN_TEMP] = BOOL_CTL       ( _panfunc, _panelID, __menu[UI_MAIN_TEMP] );
#endif
	...
	y_pos += UI_CON_HIGH;
#ifndef GCTP_USE_XFILEAPI
	MOVE_CON( _control_main[UI_MAIN_TEMP], x_pos, y_pos );
#endif
...
#ifndef GCTP_USE_XFILEAPI
	CON_SETEVENT( _control_main[UI_MAIN_TEMP], UI_TEMP_Event, this );
#endif
...
void dxUI::Update()
{
...
#ifndef GCTP_USE_XFILEAPI
	SET_INT  ( _control_main[UI_MAIN_TEMP], _options->outputTemplates );
#endif
	...
	if (_tab_no == UI_TAB_EXPORT)
	{
...
#ifndef GCTP_USE_XFILEAPI
		RENDERORREDRAW( _control_main[UI_MAIN_TEMP] );
#endif
		...
		if (_options->format == DX_FORMAT_TEXT) {
			UNGHOST_CON( _control_main[UI_MAIN_TEMP] );
		} else {
			GHOST_CON  ( _control_main[UI_MAIN_TEMP] );
		}
#endif
		if (_options->status == DX_STATUS_LAYOUT) {
...
#ifndef GCTP_USE_XFILEAPI
		ERASE_CON( _control_main[UI_MAIN_TEMP] );
#endif
...
#else
		if (_options->status  == DX_STATUS_LAYOUT &&
			_options->version == DX_VERSION_DIRECTX8) {
#endif
...
#ifndef GCTP_USE_XFILEAPI
/*
 *  UI_TEMP_Event           TEMP EVENT FUNCTION
 */
static void UI_TEMP_Event( LWControl* control, dxUI* ui )
{
	dxOptions*          opts = ui->_options;

	GET_INT( control, opts->outputTemplates );
}
#endif
